using System;
using System.Collections.Generic;
using System.Threading;

namespace LegacyRenewalApp
{
    public interface ISubscriptionPlanRepository
    {
        SubscriptionPlan GetByCode(string code);
    }
    public class SubscriptionPlanRepository : ISubscriptionPlanRepository
    {
        public static readonly Dictionary<string, SubscriptionPlan> Database = new Dictionary<string, SubscriptionPlan>
        {
            { "START", new SubscriptionPlan { Code = new StartFee(), Name = "Start", MonthlyPricePerSeat = 49m, SetupFee = 120m, IsEducationEligible = false } },
            { "PRO", new SubscriptionPlan { Code = new ProFee(), Name = "Professional", MonthlyPricePerSeat = 89m, SetupFee = 180m, IsEducationEligible = true } },
            { "ENTERPRISE", new SubscriptionPlan { Code = new EnterpriseFee(), Name = "Enterprise", MonthlyPricePerSeat = 149m, SetupFee = 300m, IsEducationEligible = false } }
        };

        public SubscriptionPlan GetByCode(string code)
        {
            int randomWaitTime = new Random().Next(500);
            Thread.Sleep(randomWaitTime);

            string normalizedCode = code.ToUpperInvariant();
            if (Database.ContainsKey(normalizedCode))
            {
                return Database[normalizedCode];
            }

            throw new ArgumentException($"Plan with code {code} does not exist");
        }
    }
}
